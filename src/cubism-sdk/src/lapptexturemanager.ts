...
export class LAppTextureManager {
  ...
  public createTextureFromPngFile(
    ...
    // 🔧 修复：添加 onerror 处理，防止纹理加载失败时模型永远卡在 WaitLoadTexture
    img.addEventListener(
      "error",
      () => {
        console.error(
          `[TextureManager] ❌ 纹理加载失败: ${fileName}（在 Android 上可能图片路径不正确或尺寸过大）`,
        );
        // 创建一个空的纹理信息并回调，防止模型卡死
        const emptyTex: WebGLTexture = gl.createTexture();
        const textureInfo: TextureInfo = new TextureInfo();
        textureInfo.fileName = fileName;
        textureInfo.width = 0;
        textureInfo.height = 0;
        textureInfo.id = emptyTex;
        textureInfo.img = img;
        textureInfo.usePremultply = usePremultiply;
        this._textures.pushBack(textureInfo);
        callback(textureInfo);
