...
export class CubismShader_WebGL {
  /**
   * 🆕 全局着色器源码缓存（跨实例共享）
   * React StrictMode 会导致 CubismFramework.dispose() 销毁着色器管理器，
   * 重新初始化时新实例需要重新加载着色器。缓存避免重复 fetch。
   */
  private static _shaderSourceCache: Map<string, string> | null = null;

  /**
   * 非同期でシェーダーをパスから読み込む
   * 🆕 优先从全局缓存读取，命中则跳过 fetch
   *
   * @param url シェーダーのURL
   *
   * @return シェーダーのソースコード
  ...
  private async loadShader(url: string): Promise<string> {
    if (!CubismShader_WebGL._shaderSourceCache) {
      CubismShader_WebGL._shaderSourceCache = new Map();
    }
    const cache = CubismShader_WebGL._shaderSourceCache;
    if (cache.has(url)) {
  ...
  public constructor() {
    ...
    // 🔧 修复：使用 Vite 可访问的绝对路径（从项目根目录）
    // 原路径 "../../Framework/Shaders/WebGL/" 从 http://localhost:1420/ 解析后为
    // http://localhost:1420/Framework/Shaders/WebGL/ → 404
    this._defaultShaderPath = "/src/cubism-sdk/Framework/Shaders/WebGL/";
