...
src/
├── components/
│   ├── HomePage.tsx       # 🏠 首页（左右分栏布局）
│   ├── Chat/
│   │   └── ChatHistory.tsx # 聊天记录面板（左栏，10vw 宽）
│   ├── Live2D/
│   │   ├── BackgroundPanel.tsx        # 背景面板（isolation:isolate 层叠上下文）
│   │   ├── Live2DCanvasNative.tsx      # 🟢 Live2D 画布组件（当前使用，原生 Cubism SDK）
│   │   ├── NativeLive2DController.ts   # 🟢 原生 SDK 控制器（当前使用）
│   │   ├── Live2DCanvas.tsx            # 🔴 旧版画布（PixiJS，已弃用）
│   │   ├── Live2DController.ts         # 🔴 旧版控制器（PixiJS，已弃用）
│   │   └── useLive2D.ts                # 🔴 旧版 Hook（已弃用）
│   ├── Voice/             # 语音相关（待实现）
│   ├── Settings/          # 设置相关（待实现）
│   ├── Common/            # 通用 UI 组件（待实现）
...
│       └── lappdefine.ts          # 配置常量
├── hooks/                 # 自定义 Hooks（useTypewriter）
├── config/                # 应用配置（live2d 模型配置）
│   ├── live2d.ts          # 模型主配置入口（导出 DEFAULT_MODEL_INFO）
│   └── live2d/            # 各模型独立配置
│       ├── README.md      #   模型配置文件规范文档 📄
│       ├── default.ts     #   re-export 当前默认模型（→ universal）
│       ├── universal.ts   #   🟢 万能自定义模型（当前默认，23纹理/38表情/3动作）
│       ├── akari.ts       #   akari 模型配置（VTube Studio, 1纹理/4表情/3动作）
│       └── moran.ts       #   moran 模型配置（VTube Studio, 4纹理/9表情/无动作）
├── services/              # API 通信服务（apiClient）
├── store/                 # Zustand 状态管理（live2dStore）
...
布局定义在 `src/components/HomePage.tsx`，入口 `src/App.tsx` 直接渲染 `<HomePage />`。
...
| `src/config/live2d/default.ts`                    | re-export 当前默认模型（→ universal）             |
| `src/config/live2d/universal.ts`                  | 🟢 万能自定义模型（当前默认）                     |
| `src/config/live2d/akari.ts`                      | akari VTS 模型                                    |
| `src/config/live2d/moran.ts`                      | moran VTS 模型                                    |
| `src/config/live2d/配置规则.md`                   | 📄 模型配置文件编写规范                           |
| `src/components/Live2D/Live2DCanvasNative.tsx`    | 🟢 原生 Cubism SDK 画布组件（当前使用）           |
...
| `src/components/Live2D/Live2DCanvas.tsx`          | 🔴 旧版画布（pixi-live2d-display，已弃用）        |
